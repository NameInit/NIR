...
dir="results/results_lz77_optlz77/"
...
for file in files:
    ...
    with open(file, 'r') as results_logic_text:
        for line in results_logic_text:
            ...
            for key in keys:
                lz77[key].append(float(temp_line[index]))
                optlz77[key].append(float(temp_line[index+4]))
                index+=1

    def show_graph(ax, key, *, title="Undefined", xlabel="Undefined", ylabel="Undefined", log_scale=False):
        ax.plot(text["size"], lz77[key], label="LZ77", color='blue', marker='o', linestyle='-')
        ax.plot(text["size"], optlz77[key], label="OPTLZ77", color='green', marker='s', linestyle='--')
